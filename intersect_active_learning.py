...
def executar_estrategia_intersecao(args):

    print(f"\nExecutando interseção entre estratégias: {args['strategy_a']} ∩ {args['strategy_b']}")
    ...
    # ------------------------------------------------------------------------
    print("Round 0")
    strategy_a.train()  # treina usando os índices já rotulados internamente
    strategy_b.train()
    ...
    # ------------------------------------------------------------------------
    # Rounds seguintes: interseção entre as duas estratégias
    # ------------------------------------------------------------------------
    for rd in range(1, args['n_round'] + 1):
        print(f"\nRound {rd} - Selecionando {args['n_query']} por interseção...")

        # Cada estratégia seleciona args['n_query'] * args['n'] candidatos
        idxs_a = strategy_a.query(args['n_query'] * args['n'])
        idxs_b = strategy_b.query(args['n_query'] * args['n'])

        n_pool = len(dataset.Y_train)
        # Interseção dos índices
        intersecao = list(set(idxs_a).intersection(set(idxs_b)))
        intersecao = [idx for idx in intersecao if 0 <= idx < n_pool]

        if len(intersecao) < args['n_query']:
            print(f"[DEBUG] Interseção retornou {len(intersecao)} amostras. Preenchendo com complemento...")
            complemento = list(set(idxs_a + idxs_b) - set(intersecao))
            complemento = [idx for idx in complemento if 0 <= idx < n_pool and idx not in intersecao]
            final_idxs = intersecao + complemento[: args['n_query'] - len(intersecao)]
        else:
            final_idxs = intersecao[: args['n_query']]
    ...
    nome_arquivo = f"resultados_intersecao_{args['strategy_a'].lower()}_{args['strategy_b'].lower()}_{args['dataset_name'].lower()}.xlsx"
